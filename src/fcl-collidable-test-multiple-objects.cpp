/*
 * fcl-collidable-test-multiple-objects.cpp
...
public:
    EdgeCallBack(const envire::core::EnvireGraph& graph, envire::core::FrameId colFrame, const MLSMapS & mls) : graph_(graph), colFrame_(colFrame), mls_(mls) {
    }
...
int main(int argc, char **argv) {
    ...
    const std::string path="./smurf/multiple_objects/smurf/multiple_objects.smurf";
    ...
    viz.write(*(graphLoader.getGraph()), "fcl-collidable-test-multiple-.dot");
    ...
    if (mapType == "sloped")
    {
        ...
        // Find out the frames which contain a collidable
        //
        ...
        for(; it != end; ++it)
        {
            ...
            if(itCols != endCols)
            {
                //allCollVertices.push_back(it->getData());
